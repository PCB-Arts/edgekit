...
def export(gpio):
	...
	if not os.path.exists(f"/sys/class/gpio/{gpio}"):
...
def unexport(gpio):
	...
	if os.path.exists(f"/sys/class/gpio/{gpio}"):
...
def set_output(gpio):
	...
	with open(f"/sys/class/gpio/{gpio}/direction", "w") as f:
		f.write("out")
...
def read_value(gpio):
	...
	with open(f"/sys/class/gpio/{gpio}/value") as f:
		return f.read()


def set_high(gpio):
	""" Set the output of a gpio to high (1) """
	with open(f"/sys/class/gpio/{gpio}/value", "w") as f:
		f.write("1")
